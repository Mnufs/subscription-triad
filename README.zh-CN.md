...
- **大上下文留在本地，但不碰工作树。**产物放在目标项目之外的用户级私有状态目录；agmsg 或内置 SQLite 传输只交换短状态和文件路径。
...
CODEX          已创建 run：<state-root>/projects/8d41…/runs/7b3c…/
...
| 不索取或落盘 Token；run 产物保留在目标工作树之外的用户级私有状态目录。 | 它不购买、打包、延长或保证任何厂商订阅。 |
...
每个项目通过 `projects/<project-hash>/` 隔离。run 位于 `runs/<uuid>/`，内置传输位于 `transport/messages.sqlite3`。目标项目不需要任何配置，也不需要修改 `.gitignore`。

可通过绝对路径环境变量 `MODEL_COMBO_STATE_DIR` 覆盖根目录；改变它以后，恢复原值之前将无法找到已有 run。在支持 POSIX 权限的平台上，状态目录使用 `0700`。产物可能包含需求、项目绝对路径、仓库上下文、计划、审核、执行输出、日志和验收报告，手动分享前仍需检查。
...
如果 `grok --oauth models` 一边提示已经登录，一边又报 `Settings fetch failed after ...`，说明 OAuth 登录存在，但在线就绪尚未确认。此时显示的默认模型可能只是缓存回退；Model Combo 会在 CLI 成功刷新前停止，不调用模型。
...
0.5.0 会把所有新 run 产物和内置传输数据移出目标工作树。已有的 `<project>/.model-combo/` 不会被自动扫描或搬移；请先用 0.4.x 完成，或在续跑前有意识地迁移。新 run 不需要项目 `.gitignore` 条目。

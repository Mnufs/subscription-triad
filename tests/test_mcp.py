...
class McpTests(unittest.TestCase):
    def _planned_run(self, root: Path):
        ...
        project.mkdir()
        created = combo_mcp.combo_core.create_run(str(project), "Task", "Acceptance", "Context")
        combo_mcp.combo_core.record_plan(created["run_dir"], "Plan")
    ...
    def test_start_prepares_only_host_approved_feature_session(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = combo_mcp.call_tool("start_provider_session", {"run_dir": run_dir})
    ...
    def test_provider_actions_prepare_stdin_without_new_host_execution(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch.object(combo_mcp.combo_core, "doctor") as doctor:
                prepared = [
                    combo_mcp.call_tool("doctor", {"run_dir": run_dir}),
                    combo_mcp.call_tool("review_plan", {"run_dir": run_dir, "effort": "max"}),
                    combo_mcp.call_tool("dispatch_grok", {"run_dir": run_dir}),
                    combo_mcp.call_tool("close_provider_session", {"run_dir": run_dir}),
                ]
    ...
    def test_continue_prepares_hash_bound_one_time_session_input(self):
        with tempfile.TemporaryDirectory() as temp:
            _project, run_dir = self._planned_run(Path(temp))
            store = combo_mcp.combo_core.RunStore(Path(run_dir))
            ...
            self.assertEqual(
                ...
                combo_mcp.combo_core.sha256_text(request_path.read_text(encoding="utf-8")),
            )
            self.assertFalse(result["requires_new_host_approval"])

    def test_start_requires_a_recorded_plan(self):
        with tempfile.TemporaryDirectory() as temp:
            created = combo_mcp.combo_core.create_run(temp, "Task", "Acceptance", "Context")
            with self.assertRaisesRegex(combo_mcp.combo_core.ComboError, "Record a canonical plan"):
                combo_mcp.call_tool("start_provider_session", {"run_dir": created["run_dir"]})

    def test_tool_errors_are_returned_without_tracebacks(self):
        response = combo_mcp.handle_request(
            {
                "jsonrpc": "2.0",
                "id": 3,
                "method": "tools/call",

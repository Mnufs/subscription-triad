#!/usr/bin/env python3
"""Run-scoped lease helpers for the Subscription Triad provider session."""
...
LEASE_PATH_ENV = "TRIAD_PROVIDER_SESSION_LEASE"
LEASE_TOKEN_ENV = "TRIAD_PROVIDER_SESSION_TOKEN"
...
def validate_run_binding(
    run_dir: str,
) -> Tuple[triad_core.RunStore, Dict[str, Any], Path]:
    store = triad_core.RunStore(Path(run_dir))
    state = store.read()
    project_value = state.get("project_root")
    if not isinstance(project_value, str):
        raise triad_core.TriadError("Run state is missing its target project.")
    project = Path(project_value).expanduser().resolve()
    if not project.is_dir():
        raise triad_core.TriadError("The run's target project is unavailable.")
    expected_runs_root = (project / ".subscription-triad" / "runs").resolve()
    if store.run_dir.parent != expected_runs_root:
        raise triad_core.TriadError("Provider session run is outside its target project.")
    run_id = state.get("run_id")
    if not isinstance(run_id, str) or run_id != store.run_dir.name or not triad_core.RUN_ID_RE.match(run_id):
        raise triad_core.TriadError("Provider session run identity is invalid.")
...
def _read_lease(path: Path) -> Dict[str, Any]:
    try:
        ...
        raise triad_core.TriadError("Provider session lease is unavailable.") from exc
    ...
    if (
        ...
        raise triad_core.TriadError("Provider session lease is not a private regular file.")
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise triad_core.TriadError("Provider session lease is unreadable.") from exc
    if not isinstance(payload, dict):
        raise triad_core.TriadError("Provider session lease has an invalid format.")
    return payload
...
class ProviderLease:
    ...
    def __init__(self, store: triad_core.RunStore):
        self.store = store
        ...
        for _attempt in range(2):
            try:
                ...
                if _existing_lease_is_active(self.path):
                    raise triad_core.TriadError("A provider session is already active for this feature.")
                try:
                    self.path.unlink()
                except OSError as exc:
                    raise triad_core.TriadError("A stale provider session lease could not be removed.") from exc
                continue
            except OSError as exc:
                raise triad_core.TriadError("Provider session lease could not be created.") from exc
            try:
                ...
                raise
            return self
        raise triad_core.TriadError("Provider session lease could not be acquired.")

    def _assert_owned(self) -> None:
        if self._fd is None:
            raise triad_core.TriadError("Provider session lease is not active.")
        try:
            path_info = self.path.lstat()
            fd_info = os.fstat(self._fd)
        except OSError as exc:
            raise triad_core.TriadError("Provider session lease ownership could not be verified.") from exc
        if (
            ...
            raise triad_core.TriadError("Provider session lease changed unexpectedly.")
    ...
    def _write_initial_record(self) -> None:
        ...
        try:
            ...
            raise triad_core.TriadError("Provider session lease could not be initialized.") from exc
    ...
    def heartbeat(self) -> None:
        ...
        try:
            ...
            raise triad_core.TriadError("Provider session lease heartbeat failed.") from exc

#!/usr/bin/env python3
"""One-approval, run-scoped host bridge for Model Combo providers.
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="combo-provider",
        description="One-approval provider session for one Model Combo feature",
    )
...
def read_bound_instructions(run_dir: str, file_name: str, expected_sha256: str) -> str:
    ...
    try:
        ...
        raise combo_core.ComboError("Continuation request is unavailable.") from exc
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
        raise combo_core.ComboError("Continuation request must be a single regular file.")
    path = supplied.resolve()
    combo_core._ensure_within(path, request_root)
    try:
        value = path.read_text(encoding="utf-8")
    except (OSError, UnicodeDecodeError) as exc:
        raise combo_core.ComboError("Continuation request is unreadable.") from exc
    actual_sha256 = combo_core.sha256_text(value)
    if not hmac.compare_digest(actual_sha256, expected_sha256.lower()):
        raise combo_core.ComboError("Continuation request changed after its session input was prepared.")
    try:
        path.unlink()
    except OSError as exc:
        raise combo_core.ComboError("Continuation request could not be consumed safely.") from exc
    return value


def _request_fields(payload: Any) -> Tuple[str, str]:
    if not isinstance(payload, dict):
        raise combo_core.ComboError("Provider session input must be a JSON object.")
    action = payload.get("action")
    request_id = payload.get("request_id")
    if not isinstance(action, str):
        raise combo_core.ComboError("Provider session action must be a string.")
    if not isinstance(request_id, str) or not request_id or len(request_id) > 128:
        raise combo_core.ComboError("Provider session request_id is invalid.")
    allowed = {
    ...
    if action not in allowed:
        raise combo_core.ComboError("Unsupported provider session action: %s." % action)
    unexpected = sorted(set(payload) - allowed[action])
    if unexpected:
        raise combo_core.ComboError("Unexpected provider session field(s): %s." % ", ".join(unexpected))
...
class ProviderSession:
    ...
    def __init__(self, store: combo_core.RunStore, project: Path):
        self.store = store
    ...
    def _require_ready(self) -> None:
        if not self.doctor_ready:
            raise combo_core.ComboError("Run doctor successfully in this provider session first.")
    ...
    def _execute(self, action: str, payload: Dict[str, Any]) -> Tuple[Any, bool]:
        ...
        if action == "doctor":
            if self.doctor_report is None:
                self.doctor_report = combo_core.doctor(str(self.project))
        ...
        if action == "review":
            ...
            return combo_core.review_plan(str(self.store.run_dir), effort=effort), False
        if action == "dispatch":
            result = combo_core.dispatch_grok(str(self.store.run_dir))
        ...
        if action == "continue":
            ...
            if not isinstance(file_name, str) or not isinstance(expected_sha256, str):
                raise combo_core.ComboError("Continuation session input is incomplete.")
            instructions = read_bound_instructions(
                str(self.store.run_dir),
                file_name,
                expected_sha256,
            )
            result = combo_core.continue_grok(str(self.store.run_dir), instructions)
            self._remember_worker(result)
            return result, False
        raise combo_core.ComboError("Unknown provider session action.")

    def _remember_worker(self, result: Any) -> None:
        pid = result.get("worker_pid") if isinstance(result, dict) else None
        if not isinstance(pid, int) or isinstance(pid, bool) or pid <= 0:
            raise combo_core.ComboError("Grok dispatch did not return a valid worker process.")
    ...
    def active_worker_pids(self) -> Set[int]:
        ...
        active = {
            ...
            if pid == current and combo_session.pid_is_alive(pid)
        }
    ...
    def terminate_active_workers(self) -> None:
        ...
        while active and time.monotonic() < deadline:
            time.sleep(0.05)
            active = {pid for pid in active if combo_session.pid_is_alive(pid)}
...
def _read_session_input(source: IO[str], messages: "queue.Queue[Any]") -> None:
    try:
        while True:
            line = source.readline(combo_session.MAX_SESSION_LINE_CHARS + 2)
            if line == "":
                break
            if len(line) > combo_session.MAX_SESSION_LINE_CHARS:
...
def serve_session(
    ...
    *,
    ...
    idle_timeout_seconds: int = combo_session.SESSION_IDLE_TIMEOUT_SECONDS,
    hard_timeout_seconds: int = combo_session.SESSION_HARD_TIMEOUT_SECONDS,
) -> int:
    store, state, project = combo_session.validate_run_binding(run_dir)
    ...
    if idle_timeout_seconds <= 0 or hard_timeout_seconds <= idle_timeout_seconds:
        raise combo_core.ComboError("Provider session timeout configuration is invalid.")
    ...
    heartbeat_errors: "queue.Queue[combo_core.ComboError]" = queue.Queue(maxsize=1)
    heartbeat_stop = threading.Event()

    def keep_lease_current() -> None:
        while not heartbeat_stop.wait(combo_session.LEASE_HEARTBEAT_SECONDS):
            try:
                lease.heartbeat()
            except combo_core.ComboError as exc:
                try:
    ...
    heartbeat = threading.Thread(
        ...
        name="combo-provider-lease-heartbeat",
        daemon=True,
    )
    heartbeat.start()
    ...
    try:
        ...
        while True:
            ...
            if item is _LINE_TOO_LONG:
                ...
                except (json.JSONDecodeError, combo_core.ComboError) as exc:
                    protocol_errors += 1
        ...
        heartbeat_stop.set()
        heartbeat.join(timeout=combo_session.LEASE_HEARTBEAT_SECONDS + 1)
        lease.close()
